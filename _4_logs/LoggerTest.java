package np.zadachi_za_vezhbanje.prv_kolokvium._4_logs;
...
class LogSystem<T extends ILog & Comparable<? super T>>{
    //TODO add instance variable(s)
    ArrayList<T> logsList;
    //TODO constructor

    public LogSystem(ArrayList<T> elements) {
        this.logsList = new ArrayList<>(elements);
    }

    void printResults() {

        //TODO define concrete log processors with lambda expressions
        LogProcessor<T> firstLogProcessor = logs -> logs.stream().filter(l->l.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList<T>::new));

        LogProcessor<T> secondLogProcessor = logs -> logs.stream().filter(l->l.getMessage().length() < 100).collect(Collectors.toCollection(ArrayList<T>::new));

        LogProcessor<T> thirdLogProcessor = logs -> logs.stream().sorted(Comparator.naturalOrder()).collect(Collectors.toCollection(ArrayList::new));
